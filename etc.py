...
def cli(doc,xpect,funs=[]):
  ...
  while args:
    ...
    if mark in "+-":
      ...
      elif flag=="ls" : [elp(k,funs[k]) for k in funs]
      elif flag=="egs": do= [(k,v) for k,v in funs.items()]
      elif not args   : print(f"W: missing argument for {flag}")
      elif flag=="eg" : do= [(k,funs[k]) for k in funs if args[0] in k]
...
def eg_s(l): return {k:v for k,v in l.items() if type(v)==fun and k[:3]=="eg_"}

def main(doc, help, funs, also=None):
  the  = obj(**cli(doc,help, funs))
