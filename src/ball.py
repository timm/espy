...
class Tab(obj):
  ...
  def __init__(i, rows=[],txt=""):
    i.txt, i.rows, i.cols, i.xs, i.ys, klass = txt, [], [], [], [], None
...
class Yell:
  ...
  def eg_Diabetes(my):
    ...
    results=Abcd()
    all=None
    for pos,row in enumerate(csv(Yell.diabetes)):
      if pos:
        klass=row[-1]
        if pos > 20:
           _,what = others[0].classify(row, my, [any])
           results.tell(klass,what.txt)
        if klass not in tabs: 
          tabs[klass] = all.clone(txt=klass)
          any,*others = list(tabs.values())
          print("space",pos,[x.txt for x in tabs.values()])
        tabs[klass].add:row
      else:
        all= Tab(rows=[row])
    #results.header()
